...
import com.loucaskreger.replanter.mixin.client.CocoaBlockInvoker;
import com.loucaskreger.replanter.mixin.client.NetherWartBlockInvoker;
    ...
    private void replaceCrop(ItemStack stack, BlockPos pos, World world) {
        ...
        if (stack.getItem() instanceof BlockItem) {
            ...
            if (block instanceof CropBlock) {
                ...
                // fully grown.
                var wartBlockInvoker = (NetherWartBlockInvoker) wartBlock;
                if ((wartBlockInvoker.invokeGetOutlineShape(state, world, pos, ShapeContext.absent()).getBoundingBox().getLengthY()
                        * 16) == 14) {
                    im.attackBlock(pos, Direction.DOWN);
                }
            } else if (block instanceof CocoaBlock cocoaBlock) {
                var cocoaBlockInvoker = (CocoaBlockInvoker) cocoaBlock;
                if ((cocoaBlockInvoker.invokeGetOutlineShape(state, world, pos, ShapeContext.absent()).getBoundingBox().getLengthY() == 0.5625)) {
